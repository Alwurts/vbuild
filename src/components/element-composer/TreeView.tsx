...
function TreeNode({ nodeKey, depth = 0 }: TreeNodeProps) {
	...
	const { openNodes, setMultipleNodesOpen } = useComposerStore(
		useShallow((state) => ({
			openNodes: state.openNodes,
			setMultipleNodesOpen: state.setMultipleNodesOpen,
		})),
	);
	...
	const isOpen = openNodes[nodeKey] || false;
			...
			<div className="relative h-8">
				{node.children && node.children.length > 0 && (
					<Button
						...
						onClick={() => {
							setMultipleNodesOpen({ [node.key]: !isOpen });
						}}
					>
						{isOpen ? (
